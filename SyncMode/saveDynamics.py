...
import random
import queue
import utils.carla_vehicle_BEV as cva
...
from utils.save_utils import save_seg, save_rgb, save_bbox_instanceSeg, save_trajectory
...
def run_simulation(args, client):
    ...
    try:
        ...
        # surround view camera's transform
        ...
        # create surround view & target camera sensor 
        ...
        fr_rgb = SensorManager(world, display_manager, 'RGBCamera',fr_transform,
                                ego_vehicle, {'sensor_tick': str(tick_sensor), 'image_size_x': str(args.w),
                                'image_size_y': str(args.h), 'fov': car_cam_fov}, display_pos=[0, 2], show = args.show)#front right
        
        
        t_sem = SensorManager(world, display_manager, 'SEMANTICCamera', t_transform, 
                                ego_vehicle, {'sensor_tick': str(tick_sensor), 'image_size_x': str(args.w),
                                'image_size_y': str(args.h), 'fov': target_cam_fov}, display_pos=[1, 0], show = args.show) # target depth
        
        t_rgb = SensorManager(world, display_manager, 'RGBCamera', t_transform, 
                                ego_vehicle, {'sensor_tick': str(tick_sensor), 'image_size_x': str(args.w),
                                'image_size_y': str(args.h), 'fov': target_cam_fov}, display_pos=[1, 1], show = args.show) # target rgb
        
        t_depth = SensorManager(world, display_manager, 'DEPTHCamera', t_transform, 
                                ego_vehicle, {'sensor_tick': str(tick_sensor), 'image_size_x': str(args.w),
        ...
        while True:
            ...
            # run simulation for fixed delta time
            nowFrame = world.tick()

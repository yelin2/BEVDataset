       ...
       # cam manage q list append
        ...
        while True:
            ...
            if time_sim >= tick_sensor:
                ...
                print(f'saved {int(cnt/51)} data: time[{round(snapshot.timestamp.elapsed_seconds, 4)}] delta[{round(snapshot.timestamp.delta_seconds, 4)}]')

                # -----------------------------
                #           Save Target
                # -----------------------------
                import matplotlib.pyplot as plt
                filename = '{0:06d}'.format(int(cnt/51))
        ...
        save_kinematics(gpss, imus, vels)
        try:
            if display_manager:
...
def save_kinematics(gpss, imus, vels):

    gps_list, imu_list, time_list = [], [], []
    assert len(gpss) == len(imus)

    # convert traj, timestamp to list
    for i, (gps, imu, vel) in enumerate(zip(gpss, imus, vels)):
